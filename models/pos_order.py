...
class PosOrder(models.Model):
    ...
    def actualizar_pedido(self,orden_id,orden,orderline,restaurante):
        ...
        if restaurante > 0:
            orders.write({'partner_id': orders.partner_id.id, 'user_id':orders.user_id.id,'customer_count': orders.customer_count})
        else:
            for order in orders: 
                order.write({'partner_id': orden['partner_id'], 'user_id':orden['user_id']})            
        lineas = self.env['pos.order.line'].search([['order_id', '=', orden_id]])
        lineas.unlink()
        for linea in orderline:
            logging.warn(linea)
